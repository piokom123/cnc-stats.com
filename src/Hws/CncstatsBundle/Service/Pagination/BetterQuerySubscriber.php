...
class BetterQuerySubscriber extends QuerySubscriber implements EventSubscriberInterface
{
    ...
    public function items(ItemsEvent $event)
    {
        if ($event->target instanceof Query) {
            ...
            if (($count = $event->target->getHint(self::HINT_COUNT)) !== false) {
                ...
                $countQuery->setDQL(\preg_replace('/SELECT(.*?)FROM/s', 'SELECT count(a) FROM', $event->target->getDQL()));

                $countResult = $countQuery->getSingleResult(Query::HYDRATE_ARRAY);
                if ($countResult[1] > 1) {
